...
Local development uses a dedicated MariaDB database and application user:
...
Qbox currently requires MariaDB 10.9.0 or newer. MariaDB 12.3 LTS is the recommended new-server target in the current Qbox documentation. XAMPP's bundled MariaDB is not supported for the playable Qbox runtime, even though it can be useful for early local repository/database scaffolding.
...
For manual setup, copy `database/init.example.sql`, replace the placeholder password locally, and run it with an administrative MariaDB account. Do not commit modified SQL files containing real passwords.
